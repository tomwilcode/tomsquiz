...
function handleAnswer(selectedIndex) {
  ...
  for (let idx = 0; idx < allOptions.length; idx += 1) {
    const el = allOptions[idx];
    el.style.pointerEvents = "none";
    el.style.backgroundColor = (idx === correctIndex ? "#c8e6c9" : "#ffcdd2");
  }
...
function showQuestion() {
  ...
  options.forEach(function(option, index) {
    ...
    listItem.addEventListener("click", function() { handleAnswer(index); });
...
function startQuiz() {
  ...
  score = 0;
  shuffledQuestions = [...quizQuestions].sort(function() { return Math.random() - 0.5; });
